   ...
   <!-- Side Navbar -->
    ...
    <!--  Top tweeter logo and menu items -->
      <div >
          ...
          <button class="text-white bg-blue rounded-full font-semibold  w-12  h-12 lg:h-auto focus:outline-none lg:w-full p-3 hover:bg-darkblue" >
         <p class="hidden lg:block" > Tweet</p>
         <i  class="fas fa-plus lg:hidden" ></i>
          </button>
      </div>

     <!--  bottom name , image and dropdowns -->
      <div class="lg:w-full relative"> 
          <button  @click="dropdown=true" class="flex items-center w-full hover:bg-lightblue rounded-full p-2 focus:outline-none">
           <img src="profile.png" class="w-10 h-10 rounded border border-lighter" />
           <div class=" hidden lg:block ml-4">
             <p class="text-sm font-bold leading-tight">Akshit Khanna </p>
             <p class="text-sm leading-tight"> akshit@twitter.com</p>
           </div>
           <i class="hidden lg:block fas fa-angle-down ml-auto text-lg"></i>
          </button>
          <!--  dropdown-->
          <div  v-if="dropdown==true" class=" bg-white  absolute bottom-0 left-0 w-64 rounded-lg shadow-md border-lightest mb-16" >
        ...
        <div class="w-full md:w-1/2 h-full overflow-y-scroll">
            ...
            <div v-for="follow in following"  :key="follow.tweet" class="w-full p-4 border-b hover:bg-lighter flex">
              <div class="flex-none mr-4">
         ...
         <div  class="md:block hidden  w-1/3 h-full border-l border-lighter py-2 px-6 overflow-y-scroll relative">
          <input  class="pl-12 rounded-full w-full p-2 bg-lighter text-sm focus:outline-none"  placeholder="Search Twitter" />
          <i class="fas fa-search absolute left-0 top-0 mt-5 ml-12 text-sm text-light" ></i>

          <div class="w-full rounded-lg bg-lightest my-2">
            <div  class="flex items-center justify-between p-3">
              <p class="text-lg font-bold">Trends for You</p>
              <i class="fas fa-cog text-lg text-blue" ></i>
            </div>

             <button v-for="trend in trending" :key="trend.title"  class="w-full flex justify-between hover:bg-lighter p-3 focus:outline-none border-t border-lighter " >
               <div >
                 <p  class="text-sm text-left leading-tight text-dark">{{trend.top}}</p>
                 <p class="font-bold text-left leading-tight">{{trend.title}}</p>
                 <p class="text-left leading-tight text-darker">{{trend.bottom}}</p>
               </div>
            ...
            <div  class=" p-3">
             ...
             <button  v-for="friend in friends" :key="friend.handle" class="w-full flex  hover:bg-lighter p-3 focus:outline-none border-t border-lighter " >
                 <img :src="`${friend.src}`" class="w-12 h-12 rounded border border-lighter" />
           <div class=" hidden lg:block ml-4">
